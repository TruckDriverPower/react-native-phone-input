...
import { AppRegistry, StyleSheet, Image, TextInput, Text, TouchableWithoutFeedback, View } from "react-native";
...
export default class PhoneInput extends Component {
  ...
  onChangePhoneNumber(number) {
    var actionAfterSetState = this.props.onChangePhoneNumber
      ? () => {
    ...
    return (
      <View style={[styles.container, this.props.style]}>
        <TouchableWithoutFeedback onPress={this.onPressFlag}>
          <Image source={Flags.get(this.state.iso2)} resizeMode="stretch" style={[styles.flag, this.props.flagStyle]} onPress={this.onPressFlag} />
        </TouchableWithoutFeedback>
        <View style={{ flex: 1, marginLeft: this.props.offset || 10 }}>
          <TextComponent autoCorrect={false} style={[styles.text, this.props.textStyle]} onChangeText={text => this.onChangePhoneNumber(text)} keyboardType="phone-pad" underlineColorAndroid="rgba(0,0,0,0)" value={this.state.formattedNumber} {...this.props.textProps} />
        </View>
